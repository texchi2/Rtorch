---
title: An R Markdown document converted from "/home/tmm/predict_Ki-66_from_HE/Inference_on_ROIs.ipynb"
output: html_document
---
# 0. R-reticulate and Miniconda environment
```{r}
# bash Miniconda......sh
# installed at /home/tmm/.local/share/r-miniconda/ => keypoint is "r-miniconda" for reticulate to use
library(reticulate)
use_miniconda("/home/tmm/.local/share/r-miniconda/")

# install torch with cuda support under r-miniconda
reticulate::conda_install(packages=c("pytorch","torchvision","torchaudio", "pytorch-cuda=11.7", "python=3.10"), method="conda", channel = c("pytorch", "nvidia"))

reticulate::conda_list("/home/tmm/.local/share/r-miniconda/bin/conda")

use_condaenv(condaenv = "r-reticulate", conda = "/home/tmm/.local/share/r-miniconda/bin/conda")

# as use_python("/home/tmm/.local/share/r-miniconda/envs/r-reticulate/bin/python")
```
...
elif device.type == "cuda": # else if
  trained_model = torch.load('new_resnet18_0.9326.model', map_location=torch.device('cuda')) # cuda
...
# adjust kenel_size=2, stride=1 into ...
conv_fc = nn.Conv2d(in_channels=512, out_channels=3, kernel_size=2, stride=5)
...
image = cv2.imread('/home/tmm/Rtorch/Test_ROIs/test_input_he/26.png') #TH1919729C1_HE.tif') # there is ok
# ok
...
```{python}
# x *** in this step: temporary switch to cpu device to avoid out of memory and R session crash
...
# 3.5 we should debug from here (R torch)
# debug by R way
```{r}
# try debug with error message
# import ipdb; ipdb.set_trace()
# import logging
# logging.basicConfig(level=logging.DEBUG)

pbar <- tqdm(total=sub_num*sub_num)
for (y in seq_len(sub_num)) {
  for (x in seq_len(sub_num)) {
    # import ipdb; ipdb.set_trace()
    # logging.debug(f"Entering tqdm with argument x as {x}")
    print(sprintf("x as %d, and y as %d in these for loops", x, y))
    # try({
      xs <- as.integer(resized_w*x/sub_num)
      ys <- as.integer(resized_h*y/sub_num)
      ws <- as.integer(resized_w/sub_num)
      hs <- as.integer(resized_h/sub_num)
      sub_image <- image[ys:(ys+hs), xs:(xs+ws), ]
      under_image_tensor <- transformation(sub_image)$float()
      image_tensor <- under_image_tensor$unsqueeze(0)

      # x temporary hold cuda
      if (torch::cuda_is_available()) {
        image_tensor <- image_tensor$to(device)
      }

      inputs <- torch::Variable(image_tensor)
      torch::no_grad({
        output1 <- convLayers(inputs)
        underscore_output2 <- conv_fc(output1)
        output2 <- nnf::softmax(underscore_output2, dim=1)
      })

      if (!is.null(X_temp)) {
        X_temp <- torch::cat(list(X_temp,output2),3)
      } else {
        X_temp <- output2
      }
      pbar$update(1)
    # }, error = function(e) {
    #   print(sprintf("An error occurred: %s", e))
    #   import ipdb; ipdb.post_mortem()
    # })
  }

  if (!is.null(Y_temp)) {
    Y_temp <- torch::cat(list(Y_temp,X_temp),2)
  } else {
    Y_temp <- X_temp
    X_temp <- NULL
  }
...
# 3.6 we should debug from here (Pytorch)
```{python}
# try debug with error message
# import ipdb; ipdb.set_trace()
#import logging
...
with tqdm(total=sub_num*sub_num) as pbar:
    for y in range(sub_num):
        for x in range(sub_num):
          #import ipdb; ipdb.set_trace()
          #logging.debug(f"Entering tqdm with argument x as {x}")
          print(f"x as {x}, and y as {y} in these for loops")
          #try:
